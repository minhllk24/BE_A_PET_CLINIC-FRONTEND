...
function FooterLink({ children, onClick, to = '#' }) {
  const className =
    "inline-flex rounded-[6px] px-[6px] py-3 text-left text-[16px] leading-[1.4] text-[rgba(0,0,0,0.87)] transition-colors hover:bg-[#FFF9C4] hover:text-[#0D47A1]"
...
const MOBILE_FOOTER_SECTIONS = [
  ['Liên kết nhanh', PRODUCT_LINKS],
  ['Dịch vụ nổi bật', SERVICE_LINKS],
  [
    'Thông tin chung',
    [
      ['Về chúng tôi', '/about'],
...
function MobileAccordionTitle({ children, open, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      aria-expanded={open}
      className="flex h-[35px] w-[320px] items-center justify-between border-b-[0.5px] border-[#0D47A1] bg-white text-left font-['Roboto'] text-[13px] font-bold leading-[1.6] tracking-[0.15px] text-[#0D47A1] transition-colors hover:bg-[#FFFDE7]"
    >
      <span>{children}</span>
      <img
...
function MobileFooterSection({ title, links, open, onToggle }) {
  return (
    <div>
      <MobileAccordionTitle open={open} onClick={onToggle}>
        {title}
      </MobileAccordionTitle>
      <div
        className="overflow-hidden transition-all duration-300 ease-premium"
        style={{ maxHeight: open ? links.length * 34 : 0 }}
      >
...
function MobileFooter() {
  ...
  return (
    <footer className="relative w-full overflow-hidden bg-white font-sans lg:hidden">
      <div className="relative mx-auto min-h-[375px] w-full max-w-[360px] bg-white pb-[24px]">
        <div className="px-[20px] pt-[20px]">
          <div className="flex w-[320px] flex-col gap-[5px]">
            {MOBILE_FOOTER_SECTIONS.map(([title, links]) => (
              <MobileFooterSection
                key={title}
                title={title}
                links={links}
          ...
          <section className="mt-[25px] flex h-[136px] w-[320px] flex-col justify-end gap-[10px]">
            <div className="flex h-[104px] w-[320px] flex-col items-center justify-center">
              <Link to="/" aria-label="Dr. Pet's House - Trang chủ">
                <img
                  src={mobileNavbarImages.footerLogo}
                  alt="Dr. Pet's House"
                  className="h-[38px] w-[88px] object-contain"
                />
              </Link>
              <h2 className="mt-[7px] whitespace-nowrap font-['Baloo_Tamma'] text-[16px] font-normal not-italic leading-[17.6px] text-[#0D47A1]">
                TRUNG TÂM CHĂM SÓC THÚ CƯNG
              </h2>
              <p className="mt-[7px] w-[320px] text-center font-['Roboto'] text-[12px] font-normal not-italic leading-[16.8px] text-[#475569]">
                Cung cấp dịch vụ khám bệnh, làm đẹp, và mua sắm sản phẩm cho thú
                cưng của bạn.
              </p>
            </div>
            <div className="flex w-[320px] items-center justify-between">
              <div className="flex min-w-0 flex-1 items-center gap-[16px]">
                {[
                  ['Facebook', mobileNavbarImages.facebook],
                  ['Zalo', mobileNavbarImages.zalo],
                  ['Instagram', mobileNavbarImages.instagram],
                  ['TikTok', mobileNavbarImages.tiktok],
                ].map(([label, icon]) => (
                  <a
                    key={label}
                    href="#"
              ...
              <a
                href="tel:0868686868"
                className="flex h-[22px] w-[108px] shrink-0 items-center justify-center rounded bg-[#FFF176] px-[6px] py-[1px] shadow-elevation transition-colors hover:bg-[#0D47A1] hover:text-white"
              >
                <span className="flex items-center justify-center gap-[3px]">
                  <img
                    src={mobileNavbarImages.phone}
                    alt=""
                    className="size-[16px] shrink-0"
                  />
                  <span className="whitespace-nowrap font-['Roboto'] text-[12px] font-bold uppercase leading-none tracking-[1px]">
                    0868686868
                  </span>
                </span>
              </a>
        ...
        <div className="absolute bottom-0 left-0 right-0 flex h-[24px] items-center justify-center bg-[#B3E5FC]">
          <p className="font-['Roboto'] text-[10px] italic leading-[1.4] text-[rgba(0,0,0,0.6)]">
  ...
  return (
    ...
    >
      <div className="flex h-6 w-full max-w-[1373px] items-center justify-between gap-6 text-[14px] text-[rgba(0,0,0,0.6)]">
        ...
        <nav
          aria-label="Liên kết chân trang tài khoản"
          className="hidden shrink-0 items-center gap-3 font-medium leading-none sm:flex"
        >
...
function Footer({ variant = 'auto' }) {
  ...
  useLayoutEffect(() => {
    ...
    const updateVariant = () => {
      setAutoVariant(
        isBlueBackground(footer.previousElementSibling) ? 'white' : 'blue',
      )
    }
      ...
      >
        ...
        <div className="relative mx-auto grid min-h-[530px] max-w-[1270px] grid-cols-[220px_144px_155px_210px_172px] items-start justify-between gap-8 px-6 pb-[62px] pt-[76px]">
          ...
          <FooterColumn title="PRODUCT">
            <ul>
              <FooterLink onClick={() => setIsPricingOpen(true)}>
                Bảng giá dịch vụ
              </FooterLink>
