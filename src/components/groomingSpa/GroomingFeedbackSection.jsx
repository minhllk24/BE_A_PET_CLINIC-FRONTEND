...
import { groomingFeedbackAssets } from "../../assets/groomingImages";
import { FEEDBACK_PAGE_COUNT, FEEDBACK_PAGES } from "../shop/feedbackData";
import { groomingTitleStyles } from "./groomingTitleStyles";

const FEEDBACK_QUOTE_LINES = 4;
const FEEDBACK_QUOTE_LINE_HEIGHT = 28;
...
function GroomingFeedbackSection() {
  ...
  const goToPage = useCallback((index) => {
    setPageIndex((prev) => {
      ...
      const forward =
        index > prev ? index - prev : index + FEEDBACK_PAGE_COUNT - prev;
  ...
  const slideOffset =
    direction === 0
      ? "translate-x-0"
      : direction > 0
        ? "animate-feedback-in-next"
        : "animate-feedback-in-prev";
  ...
  return (
    ...
    >
      <img
        src={groomingFeedbackAssets.background}
        alt=""
        className="absolute inset-0 h-full w-full object-cover"
      />

      <div className="relative z-10 px-[120px] pt-10">
        <h2 className="text-center" style={groomingTitleStyles.dark64}>
        ...
        >
          <button
            type="button"
            onClick={goPrev}
            className="btn-icon-subtle flex shrink-0 items-center justify-center rounded-lg p-1 opacity-80 transition-opacity hover:opacity-100"
            aria-label="Trang phản hồi trước"
          >
            <img
              src={groomingFeedbackAssets.carouselLeft}
              alt=""
          ...
          >
            <img
              src={groomingFeedbackAssets.carouselRight}
              alt=""
              className="h-[10px] w-[271px] object-contain"
            />
          </button>
      ...
      <img
        src={groomingFeedbackAssets.pawSmall}
        alt=""
      ...
      />
      <img
        src={groomingFeedbackAssets.pawBlue}
        alt=""
      ...
      />
      <img
        src={groomingFeedbackAssets.pawBlueSmall}
        alt=""
        className="pointer-events-none absolute left-[158.39px] top-[526.17px] h-[34px] w-[34px]"
      />
      <img
        src={groomingFeedbackAssets.pawDark}
        alt=""
        className="pointer-events-none absolute bottom-0 right-[50px] h-[87px] w-[87px]"
      />
    </section>
  );
}

export default GroomingFeedbackSection;
